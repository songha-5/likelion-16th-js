...
const bookPageNumber = parseInt(BOOK_PAGE);
console.log(bookPageNumber, typeof bookPageNumber)
...
const loanPerodPlus = +LOAN_PERIOD
console.log(loanPerodPlus, typeof loanPerodPlus)
...
const bookCount = BOOK_COUNT.toString()
console.log(bookCount, typeof bookCount)

// String()을 사용해서 BOOK_COUNT를 문자로 변환하고, 변환된 값과 값의 타입을 확인합니다.
const bookCountString = String(BOOK_COUNT)
console.log(bookCountString, typeof bookCountString)

// 빈 문자('')를 더해서 BOOK_COUNT를 문자로 변환하고, 변환된 값과 값의 타입을 확인합니다.
const bookCountBlank = '' + BOOK_COUNT
console.log(bookCountBlank, typeof bookCountBlank)
...
const priceParseFloat = parseFloat(PRICE)
console.log(priceParseFloat, typeof priceParseFloat)

// parseFloat()를 사용해서 RATING을 실수로 변환하고, 변환된 값과 값의 타입을 확인합니다.
const ratingParseFloat = parseFloat(RATING)
console.log(ratingParseFloat, typeof ratingParseFloat)
...
const stoctBoolean = Boolean(STOCK)
console.log(stoctBoolean, typeof stoctBoolean)

// Boolean()을 사용해서 SOLD_OUT을 불리언으로 변환하고, 변환된 값과 값의 타입을 확인합니다.
const soldOutBoolean = Boolean(SOLD_OUT)
console.log(soldOutBoolean, typeof soldOutBoolean)
...
const math = Math.sqrt(-1)
console.log(math)

// 1 + NaN을 사용해서 NaN을 만드세요.
const NanPlusOne = NaN + 1
console.log(NanPlusOne)

// undefined + undefined를 사용해서 NaN을 만드세요.
const undefinedPlus = undefined + undefined
console.log(undefinedPlus)

// '안녕' / 2를 사용해서 NaN을 만드세요.
const stringDivide = '안녕' / 2
console.log(stringDivide)
...
const productCalculater = parseInt(PRODUCT_PRICE) * 0.9 + '원'
console.log(productCalculater)

const price = parseInt(PRODUCT_PRICE, 10)
const discount = price * 0.9 + '원'
