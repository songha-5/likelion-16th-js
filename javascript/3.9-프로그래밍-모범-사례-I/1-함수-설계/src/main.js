...
async function handleLogin(e) {
  ...
  const submitButtonText = submitButton.querySelector('[data-button-text]')

  errorMessage.hidden = true

  submitButton.disabled = true
  submitButton.setAttribute('aria-busy', 'true')
  submitButtonText.textContent = '운명을 읽어오는 중...'
  ...
  try {
    ...
    contentDisplay.innerHTML = `
      <article data-dashboard-card style="border-top: 6px solid hsl(${themeHue}, 70%, 50%)">
        <h3>✨ ${nickname}님을 위한 메시지</h3>
        <p>"${fortune}"</p>
        <div style="margin-top: 1rem; font-weight: bold; color: hsl(${themeHue}, 60%, 40%)">
          당신은 오늘의 [${randomCompliment}] 입니다.
        </div>
      </article>
    `
    ...
    submitButtonText.textContent = '운세 뽑고 로그인하기'
  }
}
