// --------------------------------------------------------------------------
// 연습 문제: 도서관 대출 시스템
// --------------------------------------------------------------------------
// 도서관에서 책을 대출하고 반납하는 시스템을 만들어보세요.

const TOTAL_BOOKS = 145        // 도서관 전체 보유 도서
const BORROWED_TODAY = '12권'  // 오늘 대출된 책
const RETURNED_TODAY = '8권'   // 오늘 반납된 책

// BORROWED_TODAY를 숫자로 변환하세요.
const borrowedToday = parseInt(BORROWED_TODAY)
console.log(borrowedToday)

// RETURNED_TODAY를 숫자로 변환하세요.
const returnedToday = parseInt(RETURNED_TODAY)
console.log(returnedToday)

// 현재 대출 가능한 책의 수를 계산하세요.
const loansAvailable = TOTAL_BOOKS - borrowedToday + returnedToday
console.log(loansAvailable)


// 대출 가능한 책이 100권 이상인지 확인하세요.
const loansAvailableBook = loansAvailable >= 100
console.log(loansAvailableBook)

// 결과를 출력하세요.
// console.log('오늘 대출된 책 =', ___권)
console.log('오늘 대출된 책 =', BORROWED_TODAY)
// console.log('오늘 반납됩 책 =', ___권)
console.log('오늘 반납됩 책 =', RETURNED_TODAY)
// console.log('현재 대출 가능한 책의 수 = ', ___권)
console.log('현재 대출 가능한 책의 수 = ', loansAvailable)
// console.log('대출 가능한 책이 100권 이상 =', 참/거짓)
console.log('대출 가능한 책이 100권 이상 =', loansAvailableBook)

// --------------------------------------------------------------------------
// 연습 문제: 온라인 서점 할인 계산
// --------------------------------------------------------------------------
// 온라인 서점에서 책을 구매할 때 할인을 적용하는 프로그램을 만들어보세요.

const BOOK_PRICE = '18500원'   // 책 정가
const BOOK_RATING = '4.5점'    // 책 평점
let cart_count = 0              // 장바구니 수량

// BOOK_PRICE를 숫자로 변환하세요.
const bookPriceNumber = parseInt(BOOK_PRICE)
console.log(bookPriceNumber)

// 장바구니에 책을 3권 추가하세요.
cart_count = 3
console.log(cart_count)

// 총 금액을 계산하세요. (책 가격 × 수량)
const totalPrice = bookPriceNumber * cart_count
console.log(totalPrice)

// 10% 할인된 금액을 계산하세요.
const discount = totalPrice - (totalPrice * 0.1)
console.log(discount)

// 최종 결제 금액을 계산하세요.
const finalPrice = discount
console.log(finalPrice)

// 결과를 출력하세요.
console.log('장바구니 수량 =', cart_count+'권')
console.log('할인 전 금액 =', totalPrice+'원')
console.log('할인된 금액 =', discount+'원')
console.log('최종 결제 금액 =', finalPrice+'원')


// --------------------------------------------------------------------------
// 연습 문제: 도서 페이지 진행률 계산
// --------------------------------------------------------------------------
// 책을 읽는 진행률을 계산하는 프로그램을 만들어보세요.

const BOOK_PAGE = '256쪽'      // 전체 페이지
const SHELF_NUMBER = '12'      // 서가 번호
let current_page = 0            // 현재 읽은 페이지

// BOOK_PAGE를 숫자로 변환하세요.
const page = parseInt(BOOK_PAGE)
console.log(page)

// 오늘 64페이지를 읽었습니다. current_page에 더하세요.
current_page = 64
console.log(current_page)

// 내일 48페이지를 더 읽을 예정입니다. current_page에 더하세요.
current_page = 64 + 48
console.log(current_page)

// 읽은 페이지가 전체의 절반 이상인지 확인하세요.
const half = Boolean(BOOK_PRICE) >= current_page
console.log(half)

// 남은 페이지를 계산하세요.
const restPage = parseInt(BOOK_PAGE) - current_page
console.log(restPage)

// 진행률을 계산하세요.
const progress =  (current_page / page) * 100 // 다시보기
console.log(progress)

// 하루에 32페이지씩 읽는다면, 남은 책을 다 읽는데 며칠이 걸릴까요?
const expectation =  restPage / 32
console.log(expectation)

// 결과를 출력하세요.
// console.log('전체 페이지 =', ___쪽)
// console.log('현재 페이지 =', ___쪽)
// console.log('남은 페이지 =', ___쪽)
// console.log('진행률 =', ___%')
// console.log('절반 이상 읽음 =', ___)
// console.log('완독까지 남은 일수 =', ___일)


// --------------------------------------------------------------------------
// 연습 문제: 도서관 회원 포인트 시스템
// --------------------------------------------------------------------------
// 도서관에서 책을 대출하면 포인트를 적립하는 시스템입니다.

const MEMBER_ID = '2024'       // 회원 번호
let points = 0                 // 현재 포인트

// 책 1권 대출 시, 100포인트를 적립합니다. (3권 대출)
points += 100
points += 100
points += 100

console.log(points)


// 포인트가 500점 이상이면 '우수 회원'입니다.
const excellence = 500 >= points 

// 회원 번호가 숫자 2024와 일치하는지 확인하세요.
const memberCurrent = MEMBER_ID === 2024

// 포인트로 책 1권을 교환하려면 250포인트가 필요합니다.
// 현재 포인트로 몇 권을 교환할 수 있나요?
let exchange_books = Math.floor(points / 250)
console.log(exchange_books)

// 교환 후 남은 포인트는 얼마인가요?
let remaining_points = points - 250 * exchange_books
console.log(remaining_points)

// 결과를 출력하세요.
// console.log('회원 번호:', ___)
// console.log('현재 포인트:', ___점)
// console.log('우수 회원:', ___)
// console.log('교환 가능 권수:', ___권)
// console.log('교환 후 남은 포인트:', ___점)