...
function renderDashboard(user, data) {
  ...
  const carts = data.cartsData.value?.carts
  const products = data.productsData.value?.products
  const posts = data.postsData.value?.posts
  const todos = data.todosData.value?.todos
  console.log(
    carts,
    products,
    posts,
